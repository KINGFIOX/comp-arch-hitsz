...
#pragma omp parallel num_threads(real_thread_num) default(none) shared(C) firstprivate(A, B, M, N, K, KERNEL_MR, KERNEL_NR, m_thread, n_thread)
  {
    ...
    for (int k = 0; k < K; k++) {
      memcpy(B_padding + k * (dim_n_per_thread + n_padding), B + thread_n_start + k * N, (thread_n_end - thread_n_start) * sizeof(float));
    }

    for (int m = thread_m_start; m < thread_m_end; m++) {
      memcpy(C_padding + (m - thread_m_start) * (dim_n_per_thread + n_padding), C + m * N + thread_n_start, (thread_n_end - thread_n_start) * sizeof(float));
    }

    // 调用内核计算
    gemm_kernel_opt_avx(C_padding, A_padding, B_padding, (dim_m_per_thread + m_padding), (dim_n_per_thread + n_padding), K);

    // 拷回数据
    for (int m = thread_m_start; m < thread_m_end; m++) {
      memcpy(C + m * N + thread_n_start, C_padding + (m - thread_m_start) * (dim_n_per_thread + n_padding), (thread_n_end - thread_n_start) * sizeof(float));
    }
