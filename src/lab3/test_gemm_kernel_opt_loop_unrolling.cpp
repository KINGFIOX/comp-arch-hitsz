...
TEST(gemm_kernel_opt_loop_unrolling, test0) {
  // 测试正确性
  ...
  // 调用矩阵乘法
  gemm_kernel_opt_loop_unrolling(C, A, B, M, N, K);

  // 校验结果矩阵
  for (int m = 0; m < M; m++) {
    for (int n = 0; n < N; n++) {
      ASSERT_NEAR(C[m * N + n], C_check[m * N + n], 1e-5)
          << "C[" << m * N + n << "](m=" << m << ", M=" << M << ", n=" << n << ", N=" << N << ", K=" << K << ") is abnormal!" << endl;
    }
  }
...
TEST(gemm_kernel_opt_loop_unrolling, test1) {
  // 测试正确性
  const int M = 16, N = 128, K = 64;
...
TEST(gemm_kernel_opt_loop_unrolling, test2) {
  // 测试正确性
