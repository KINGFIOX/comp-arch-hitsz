...
int main(int argc, char *argv[]) {
  ...
  double baseline_benchmark = 0, opt_loop_benchmark = 0;
  ...
  assert(ss >> N);
  /**结束读取命令行参数**/
  cout << "--- Performance before prefetch optimization ---" << endl;
  baseline_benchmark = do_gemm(M, K, N, perform_gemm_baseline);
  cout << "--- Performance for after prefetch optimization ---" << endl;
  opt_loop_benchmark = do_gemm(M, K, N, perform_gemm_opt_prefetch);
  cout << "----------------------------" << endl;
  cout << "Performance difference(Gflops): " << (opt_loop_benchmark - baseline_benchmark) << endl;
  return 0;
}
