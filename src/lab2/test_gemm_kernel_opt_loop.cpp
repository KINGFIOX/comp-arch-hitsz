...
TEST(gemm_kernel_opt_loop, test0) {
  // 测试正确性
  ...
  // 调用矩阵乘法
  gemm_kernel_opt_loop(C, A, B, M, N, K);
...
TEST(gemm_kernel_opt_loop, test1) {
  // 测试正确性
...
TEST(gemm_kernel_opt_loop, test2) {
  // 测试正确性
  const int M = 128, N = 256, K = 16;
...
TEST(gemm_kernel_opt_loop, test3) {
  // 测试正确性
